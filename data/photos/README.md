...
- **RU:** Единственные метки обработки в кадре — рукописные записки `irradiated`/`control` на паре гороха 04.06 (id 20–21). Метка `irradiated-ch17` на id 22–23 — только свободный текст манифеста ('вся облучения 17'), без метки в кадре и без парного контроля. Для CH19 и CH21 фотографий нет.
- **RU:** В контрольном кадре 04.06 (id 21) на лотке микрозелени обильная белая плесень. Это неконтролируемый конфаундер, а не эффект обработки; сравнения по нему делать нельзя.
